...
import { testimonials } from '@/content/gym';
...
export function Testimonials() {
  ...
  return (
    <section className="relative overflow-hidden bg-concrete py-24 sm:py-32">
      <div
        className="absolute inset-0 bg-[radial-gradient(circle_at_20%_10%,rgba(242,74,22,0.10),transparent_50%)]"
        aria-hidden
      />
        ...
        <div className="mx-auto flex max-w-3xl flex-col items-center text-center">
          ...
          <div className="relative mt-8 min-h-[220px] w-full">
            <AnimatePresence mode="wait">
              ...
              >
                <div className="flex gap-1 text-forge-500" aria-hidden>
              ...
              {testimonials.map((t, i) => (
                <button
                  ...
                  className={`h-2.5 rounded-full transition-all ${
                    i === index
                      ? 'w-7 bg-forge-500'
                      : 'w-2.5 bg-forge-200 hover:bg-forge-300'
                  }`}
                />
              ))}
            </div>
...
function NavButton({ dir, onClick }: { dir: -1 | 1; onClick: () => void }) {
  return (
    <button
      ...
      className="flex h-10 w-10 items-center justify-center rounded-full border border-ink/15 bg-white text-ink transition-colors hover:border-forge-400 hover:text-forge-600"
    >
      <svg
        width="18"
        height="18"
