            ...
            {business.social.map((s) => {
              ...
              return (
                <a
                  ...
                  aria-label={s.label}
                  className="flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-white/80 transition-colors hover:bg-sky-500 hover:text-white"
                >
                  <Icon />
                </a>
              );
        ...
        <nav aria-label="Footer" className="flex flex-col gap-3">
          ...
          {navLinks.map((l) => (
            <a
              key={l.href}
              href={l.href}
              className="text-sm text-white/70 transition-colors hover:text-mint-200"
            >
              {l.label}
            </a>
          ))}
